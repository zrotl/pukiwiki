...
if (!defined('PLUGIN_RECAPTCHA3_ALLOW_USERS'))    define('PLUGIN_RECAPTCHA3_ALLOW_USERS',    array(
    // 'admin',
));   // reCAPTCHAなしでのPOSTを許可するユーザーを指定
if (!defined('PLUGIN_RECAPTCHA3_ALLOW_GROUPS'))    define('PLUGIN_RECAPTCHA3_ALLOW_GROUPS',    array(
    // 'valid-user',
    'valid-user',
));   // reCAPTCHAなしでのPOSTを許可するグループを指定
...
function plugin_recaptcha3_action() {
    ...
    // ログインしていたら無視する
    global $auth_user, $auth_user_groups, $auth_user_fullname;
    if ($auth_user !== '' || in_array($_SESSION['authenticated_user'], PLUGIN_RECAPTCHA3_ALLOW_USERS)) array('msg' => 'recaptcha3', 'body' => $result);
    
    foreach ($auth_user_groups as $group) {
        if ($group !== '' || in_array($group, PLUGIN_RECAPTCHA3_ALLOW_GROUPS)) array('msg' => 'recaptcha3', 'body' => $result);
    }
